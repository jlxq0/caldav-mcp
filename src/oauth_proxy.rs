...
impl OAuthProxyState {
    pub fn new(
        ...
        resource_url: &str,
        allowed_redirect_uris: Vec<String>,
        stalwart_audience: impl Into<String>,
    ) -> Self {
        ...
        Self {
            inner: Arc::new(Inner {
                ...
                stalwart_audience: stalwart_audience.into(),
...
/// API resource for this proxy is `https://caldav-mcp.kampong.social`
/// (the origin). RFC 9728 metadata stays `{origin}/mcp`; only the
/// Logto `resource` parameter is rewritten.
///
/// A non-URI `stalwart_audience` (including `stalwart`) is refused: we
/// never emit it.
fn apply_stalwart_resource(pairs: &mut Vec<(String, String)>, stalwart_audience: &str) {
    if !crate::config::is_absolute_http_uri(stalwart_audience) {
        return;
    }
    let mut saw = false;
    for (k, v) in pairs.iter_mut() {
        if k == "resource" {
            stalwart_audience.clone_into(v);
            saw = true;
        }
    }
    if !saw {
        pairs.push(("resource".to_owned(), stalwart_audience.to_owned()));
    }
}
...
pub async fn authorize(State(st): State<OAuthProxyState>, RawQuery(q): RawQuery) -> Response {
    ...
    apply_stalwart_resource(&mut pairs, &st.inner.stalwart_audience);
...
mod tests {
    ...
    fn callback_url_derived_from_resource() {
        ...
        assert_eq!(
            ...
            "https://caldav-mcp.example.test/oauth/callback"
        );
    ...
    async fn authorize_forwards_only_allowed_redirect_uri_to_pending_state() {
        ...
        let response = authorize(
            State(st.clone()),
            RawQuery(Some(
                "client_id=abc&redirect_uri=https%3A%2F%2Fclaude.ai%2Fapi%2Fmcp%2Fauth_callback&state=client-state&resource=https%3A%2F%2Fcaldav-mcp.example.test%2F"
                    .to_owned(),
            )),
        )
        .await;
        ...
        assert_eq!(
            ...
            Some("https://caldav-mcp.example.test/oauth/callback")
        );
        let resource = params.get("resource").map(String::as_str);
        assert_eq!(resource, Some("https://caldav-mcp.example.test"));
    ...
    #[test]
    fn apply_stalwart_resource_rewrites_origin_and_mcp_path_to_absolute_uri() {
        const URI: &str = "https://caldav-mcp.kampong.social";
        let mut pairs = vec![(
            "resource".to_owned(),
            "https://caldav-mcp.kampong.social/mcp".to_owned(),
        )];
        apply_stalwart_resource(&mut pairs, URI);
        assert_eq!(pairs, vec![("resource".to_owned(), URI.to_owned())]);
        assert!(Url::parse(&pairs[0].1).unwrap().has_host());
        let mut pairs = vec![(
            "resource".to_owned(),
            "https://caldav-mcp.kampong.social/".to_owned(),
        )];
        apply_stalwart_resource(&mut pairs, URI);
        assert_eq!(pairs, vec![("resource".to_owned(), URI.to_owned())]);
    }
    ...
    #[test]
    fn apply_stalwart_resource_inserts_absolute_uri_when_missing() {
        const URI: &str = "https://caldav-mcp.kampong.social";
    ...
    #[test]
    fn apply_stalwart_resource_never_sends_bare_stalwart() {
        let mut pairs = vec![(
            "resource".to_owned(),
            "https://caldav-mcp.kampong.social/mcp".to_owned(),
        )];
        apply_stalwart_resource(&mut pairs, "stalwart");
        let resource = pairs
            .iter()
            .find(|(k, _)| k == "resource")
            .map(|(_, v)| v.as_str());
        ...
        assert_eq!(resource, Some("https://caldav-mcp.kampong.social/mcp"));
    }
