...
//! public-SPA client (`CALDAV_MCP_DCR_CLIENT_ID`, redirect URIs already
...
/// `CALDAV_MCP_RESOURCE_URL` to include `/mcp`.
...
mod tests {
    ...
    fn protected_resource_points_at_self() {
        ...
        assert_eq!(m.resource, "https://caldav-mcp.example.test/mcp");
        ...
        assert_eq!(
            ...
            vec!["https://caldav-mcp.example.test"]
        );
    ...
    fn as_metadata_is_same_origin_with_jwks_on_logto() {
        ...
        assert_eq!(m.issuer, "https://caldav-mcp.example.test");
        // authorize/token are on OUR origin (proxied); jwks stays on Logto.
        assert_eq!(
            m.authorization_endpoint,
            "https://caldav-mcp.example.test/authorize"
        );
        assert_eq!(m.token_endpoint, "https://caldav-mcp.example.test/token");
    ...
    fn as_metadata_advertises_registration_when_configured() {
        ...
        assert_eq!(
            ...
            Some("https://caldav-mcp.example.test/register")
        );
    }
    ...
    #[tokio::test]
    async fn register_accepts_cursor_and_grok_four_uri_set_in_one_body() {
        ...
        cfg.oauth_redirect_uris.clone_from(&redirect_uris);
        let body = json!({ "redirect_uris": redirect_uris });
    ...
    fn www_authenticate_includes_resource_metadata_url() {
        let h = www_authenticate_header("https://caldav-mcp.example.test");
        assert!(h.starts_with("Bearer "));
        assert!(h.contains(
            r#"resource_metadata="https://caldav-mcp.example.test/.well-known/oauth-protected-resource/mcp""#
        ));
    }
}
